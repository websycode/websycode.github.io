FR.components.gridView = Ext.extend(Ext.grid.GridView, {
	...
	prepareData: function(r) {
		...
		if (!r.data.isFolder) {
			...
			r.data.filenameHTML = name +'<span>'+ ext +'</span>';
		} else {
		...
		if (this.isListViewStyle()) {
			if (r.data.isFolder) {
				r.data.iconHTML = '<i class="fa '+folderIconCls+'"></i>';
			} else {
				if (this.viewMode == 'music') {
					r.data.iconHTML = '<img src="'+FR.UI.getFileIconURL('audio.png')+'" width="26" height="26" />';
				} else {
		...
		if (r.data.isFolder) {
			...
			html = labelHTML +
				'<table class="'+cls+'" cellspacing="0"><tr>' +
					'<td class="icon"><i class="fa fa-lg '+folderIconCls+'"></i></td>' +
					'<td><div class="name" ext:qtip="'+r.data.filename+'">' +
						r.data.filename +
					'</div></td>'+
				'</tr></table>'+ iconsHolder;
		} else {
			...
			if (this.viewMode == 'thumbnails') {
				...
				html =
					'<div class="tmbItem">' +
					labelHTML +
					'<div class="tmbInner'+cls+'" id="'+itemId+'" style="'+iconStyle+'">'+ iconsHolder +'<div class="selOverlay"></div></div>' +
					'<div class="name" ext:qtip="'+r.data.filename+'&lt;br&gt; '+r.data.nice_filesize+'">'+r.data.filenameHTML+'</div>' +
					'</div>';
			} else {
				html = '<div class="tmbItem" style="'+iconStyle+'" ext:qtip="'+r.data.filename+'">'+labelHTML+'</div>';
			}
		}
		if (r.data.isFolder) {
