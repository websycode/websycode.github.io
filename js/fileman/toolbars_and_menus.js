FR.initToolbar = function() {
	...
	FR.UI.actions = {
		...
		info: new Ext.Button({
			...
			overflowText: FR.T((User.perms.file_history)?'Details and activity':'Details'),
			toggleHandler: function(btn, toggled) {
				if (toggled) {
		...
		toggleViewList: new Ext.Action({
			...
			menu: {
				...
				items: [
					...
					'-',
					{iconCls: 'fa-picture-o', text: FR.T('Photos'), mode: 'photos'},
					{iconCls: 'fa-music', text: FR.T('Music'), mode: 'music'}
				]
			}
		}),
	...
	FR.UI.headerTBar = new Ext.Toolbar({
		height: 70, plain: true, cls: 'headerTbar x-unselectable', items: items,
	...
	a.preview = new FR.ContextAction({
		...
		action: 'preview', requires: ['single', 'file', {skip_sections: ['trash']}, 'download']
	});
	...
	a.shareWithUsers = new FR.ContextAction({
		text: FR.T('With users'), iconCls: 'fa-user-plus', action: 'shareWithUsers', requires: [
			'single', 'non-empty', 'download', 'share', 'isNotCollection'
		],
		isShareItem: true
	});
	...
	a.props = new FR.ContextAction({
		text: FR.T('Control Panel'), iconCls: 'fa-cogs', action: 'props', requires: [
			function (){return User.isAdmin;}, 'single',
			function(cm) {
				if (['recent', 'myfiles', 'userWithShares', 'sharedFolder', 'starred', 'shares', 'webLinked', 'collection'].indexOf(cm.section) != -1) {
	...
	a.more = new FR.ContextAction({
		...
		menu: [a.metadata, a.versioning, a.notifications, a.alog, a.props]
	});
	...
	FR.UI.contextMenu = new FR.components.ContextMenu({
		items: [
			a.newOpt, a.newFolder,
				a.s0,
			a.locate, a.refresh, a.selectAll, a.sortItems,
				a.s1,
			a.addTags2, a.unweblink, a.removeFromCollection,
				a.s2,
			a.download, a.preview, a.openWith,
				a.s3,
